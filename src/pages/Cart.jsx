...
import {clearPizzaFromCart, removeCartItem, decCartItem, incCartItem} from '../bll/cart-reducer'
import cartEmptyImg from '../assets/img/empty-cart.png'

export const Cart = () => {
    const dispatch = useDispatch()
    const cartItems = useSelector(state => state.cart)
    const pizzas = Object.keys(cartItems.items).map(key => {
        return cartItems.items[key].currentPizzaItems[0]
    })

    const clearCart = () => {
        if (window.confirm('Вы действительно хотите очистить корзину?')){
            dispatch(clearPizzaFromCart())
        }
    }
    const removeItem = (id) => {
    ...
    const decItem = (id) => { dispatch(decCartItem(id)) }
    const incItem = (id) => { dispatch(incCartItem(id)) }
                        ...
                        <div className="cart">
                            ...
                            <div className="content__items">
                                {pizzas.map(pizza =>  <CartItem
                                    ...
                                    totalPrice={cartItems.items[pizza.id].totalPriceItem}
                                    totalCount={cartItems.items[pizza.id].currentPizzaItems.length}
                                />)}
                            ...
                            <div className="cart__bottom">
                                <div className="cart__bottom-details">
                                    <span> Всего пицц: <b>{cartItems.totalPizzasCount}</b> </span>
                                    <span> Сумма заказа: <b>{cartItems.totalPrice} ₽</b> </span>
                                </div>
